-- =============================================================================
-- 0030 - Importacao aceita o codigo de barras da planilha
-- O relatorio do sistema antigo do mercado traz o EAN de quase todo produto
-- (4.888 de 5.027 na carga de 28/08/2026). A importacao ignorava essa coluna,
-- e o codigo so entrava se alguem escaneasse produto por produto no balcao.
--
-- O cuidado todo esta no conflito: products.barcode e UNIQUE quando
-- preenchido, e uma unica colisao abortaria a transacao inteira - ou seja,
-- um codigo repetido na planilha derrubaria a importacao dos outros 5 mil
-- produtos. Por isso o codigo aqui NUNCA e obrigatorio: quando conflita, o
-- produto entra sem codigo e a linha volta marcada, em vez de estourar.
-- =============================================================================
...
begin
  ...
  begin
    ...
    loop
      ...
      -- So digitos, e so no tamanho de codigo de barras de verdade. Codigo
      -- interno curto do sistema antigo ("14", "19") nao e EAN e entraria
      -- so para atrapalhar a leitura por camera.
      ...
      -- O codigo so vale se ninguem mais estiver usando. Conferir antes e o
      -- que impede a colisao de derrubar a transacao inteira.
      v_barcode_ok := null;
      ...
      if v_existing_id is not null then
        update public.products
           set category_id    = v_cat_id,
               ...
               price           = v_price,
               -- Nao sobrescreve codigo ja cadastrado: quem escaneou o
               -- produto no balcao tinha a embalagem na mao, a planilha nao.
