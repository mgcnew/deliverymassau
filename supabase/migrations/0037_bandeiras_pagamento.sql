-- =============================================================================
-- 0037 - Bandeiras aceitas por forma de pagamento, e a do cliente no voucher
--
-- O cliente perguntava "aceita Alelo?", "aceita Elo?" e o checkout nao tinha
-- resposta. Agora cada forma de pagamento guarda as bandeiras que o mercado
-- aceita (cartoes e voucher), editadas na aba Pagamentos das configuracoes.
--
-- No voucher o cliente ESCOLHE a bandeira dele entre as aceitas: o pagamento
-- e na entrega, na maquininha do entregador, e "Voucher - Alelo" no pedido diz
-- a ele o que esperar. Nos cartoes a lista e so informativa.
-- =============================================================================

alter table public.payment_methods
  add column if not exists brands text[] not null default '{}';

comment on column public.payment_methods.brands is
  'Bandeiras aceitas (ex.: Visa, Elo, Alelo). Mostradas no checkout; no voucher o cliente escolhe uma delas.';

-- Entra DESLIGADO: voucher ativo sem bandeira nenhuma deixaria o cliente sem
-- o que escolher. O admin cadastra as bandeiras e liga na aba Pagamentos.
insert into public.payment_methods (code, label, is_active, sort_order)
values ('voucher', 'Vale-alimentacao / refeicao', false, 5)
...
alter table public.orders
  add constraint orders_payment_brand_ck
  check (payment_brand is null or length(btrim(payment_brand)) between 1 and 60);

comment on column public.orders.payment_brand is
  'Bandeira escolhida pelo cliente (hoje so no voucher). Snapshot: nao muda se a lista de bandeiras mudar depois.';

-- Configuracoes publicas: bandeiras junto de cada forma. Voucher sem bandeira
-- nao aparece mesmo que alguem o ative - nao teria o que escolher.
...
-- Pagina do pedido e nota do cliente: mostram "Voucher - Alelo".
...
-- Criacao do pedido: igual a versao da 0023, mais a bandeira do voucher. A
-- bandeira precisa estar entre as aceitas NA HORA do pedido - o navegador so
-- mostra as opcoes, quem decide e o banco.
